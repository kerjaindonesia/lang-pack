...
/**
 * Strings for component 'auth_ldap', language 'id', version '3.9'.
 *
 * @package     auth_ldap
 * @category    string
...
$string['auth_ntlmsso_remoteuserformat'] = 'Jika Anda telah memilih \'NTLM\' di \'Jenis otentikasi\', Anda dapat menentukan format nama pengguna jarak jauh di sini. Jika Anda membiarkannya kosong, format default DOMAIN\\namapengguna akan digunakan. Anda dapat menggunakan placeholder <b>%domain%</b> opsional untuk menentukan di mana nama domain muncul, dan placeholder <b>%username%</b> wajib untuk menentukan di mana nama pengguna muncul. <br /><br />Beberapa format yang banyak digunakan adalah <tt>%domain%\\%username%</tt> (default MS Windows), <tt>%domain%/%username%</tt>,<tt>%domain%+%username%</tt> and just <tt>%username%</tt>(jika tidak ada bagian domain).';
...
$string['updateremfailfield'] = 'Gagal memperbarui LDAP dengan bidang yang tidak ada (\'{$a->ldapkey}\'). Kunci ({$a->key}) - nilai Moodle lama: \'{$a->ouvalue}\' nilai baru: \'{$a->nuvalue}\'';
